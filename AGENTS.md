...
|---|---|
| `core/controller.py` | Pure decision engine: one tick from `HouseSnapshot` → `Decision` (mode arbitration, demand/helper selection, min-on/off guards, tracking-delta adaptation, shedding, fan assist, window suggestions). Orchestrates extracted helpers below. |
| `core/cop_timing.py` | COP-timed efficiency-band widen (center fixed): band COP vs forecast, hysteretic `cop_widen_k`, Boost stretches only the conditioning edge. |
| `core/regime.py` | Operating regime: `ventilate` / `continuous` / `cycling`, 15 min dwell, night-ventilate gating. |
| `core/plant.py` | Compressor run clock (`plant_compress_since`), plant min_on, per-zone chatter / mode-change guards. |
| `core/thermal.py` | Per-zone 1R1C `ThermalModel`: RLS fits of k_out / k_mix per door regime, effective capacitance, diurnal disturbance, `q_transient` overlay (off-period innovation; not written into RLS), forecast-conditioned solar/rain on `q_eff`, sensible power, COP estimation. Power is fit-consistent (`UA = c_eff·k`, `Q_ac = c_eff·(dT/dt − free_float_rate)`); outdoor *volume* flow for moisture stays `k_out·V`. |
...
| `core/head_depth.py` | `HeadDepth`: current signed depth plus distinct chase floor, hold_since, hold_ref, probe clocks. `zone_park_margin` is the redundant d>0 mirror. |
...
| `core/types.py` | All dataclasses: `Settings`, `ZoneSnapshot`, `HouseSnapshot`, `Command` (signed `head_depth_k` only — HA/COP still use the token `"park"` for d>0), `ControllerState` (+ its persistence round-trip). |
